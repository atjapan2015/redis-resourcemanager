...
resource "oci_core_instance" "redis_master" {
  ...
  availability_domain = length(data.oci_identity_availability_domains.ads.availability_domains) == 1 ? lookup(data.oci_identity_availability_domains.availability_domains.availability_domains[0],"name") : lookup(data.oci_identity_availability_domains.ads.availability_domains[(count.index%3)],"name")
