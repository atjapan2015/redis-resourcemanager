...
resource "oci_core_instance" "redis_replica" {
  ...
  fault_domain        = "FAULT-DOMAIN-${(count.index + ((var.redis_deployment_type == "Master Slave") ? var.redis_masterslave_master_count : var.redis_rediscluster_shard_count)) % 3 + 1}"
